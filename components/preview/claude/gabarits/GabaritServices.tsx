...
const page = PREVIEW_PAGES.find((item) => item.id === "services");

const PROJETS = [
  {
    label: "Je vends",
    detail: "Le dossier complet avant compromis : rien ne bloque chez le notaire.",
    icon: HomeIcon,
  },
  {
    label: "Je loue",
    detail: "Bail sécurisé, locataire informé, obligations à jour.",
    icon: FileTextIcon,
  },
  {
    label: "Je fais des travaux",
    detail: "Amiante et plomb repérés avant l’ouverture du chantier.",
    icon: WrenchIcon,
  },
  {
    label: "Je gère une copropriété",
    detail: "Parties communes et obligations collectives planifiées.",
    icon: Building2Icon,
  },
] as const;
...
const METHODE = [
  {
    etape: "Décrire",
    detail: "Votre bien en 2 minutes, en ligne ou au téléphone. Devis sous 2 h ouvrées.",
  },
  {
    etape: "Intervenir",
    detail: "Un technicien certifié, un seul passage pour tous les contrôles compatibles.",
  },
  {
    etape: "Expliquer",
    detail: "Le rapport commenté, et une équipe joignable après la remise.",
  },
] as const;

export function GabaritServices() {
  if (!page) return null;
    ...
    <div className="bg-white">
      ...
      <section className="relative overflow-hidden bg-[color:var(--color-home-ink)]">
        <Image
          ...
          alt="Immeuble résidentiel couvert par les diagnostics Servicimmo"
          fill
          sizes="100vw"
        ...
        <p className="absolute top-6 right-6 hidden font-mono text-[10px] font-bold tracking-[.2em] text-white/55 uppercase lg:block">
          Catalogue {page.stat} expertises · Indre-et-Loire
        </p>
        <div className="relative mx-auto grid max-w-[var(--container,1280px)] items-center gap-12 px-6 pt-16 pb-24 md:px-8 lg:grid-cols-[1.05fr_.95fr] lg:gap-14 lg:pt-24 lg:pb-32">
          <div>
            ...
            <div className="mt-8 flex flex-wrap items-center gap-5">
              ...
              <p className="text-[12.5px] font-semibold text-white/70">
                {page.stat} {page.statLabel} · une seule intervention
              </p>
            </div>
          </div>
          <div className="relative hidden rounded-[18px] border border-white/15 bg-[color:var(--color-home-ink)]/72 p-8 backdrop-blur-md lg:block">
              ...
              {page.items.map((item, index) => (
                <div key={item.title} className="flex items-center gap-4 py-4">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="mt-0.5 font-[family-name:var(--font-sora)] text-[15px] font-bold text-white">
                      {item.title}
                    </p>
                  </div>
            ...
            <p className="mt-4 border-t border-[color:var(--color-si-lime)]/40 pt-3 font-mono text-[9.5px] tracking-[.14em] text-white/45 uppercase">
              Familles regroupables en un seul passage
            </p>
          </div>
        </div>
      </section>
      ...
      {/* Catalogue — forme Codex validée : panneau encre + lignes blanches */}
      <section
        id="catalogue"
      ...
      >
        <div className="grid overflow-hidden rounded-[16px] bg-[color:var(--color-home-ink)] lg:grid-cols-[340px_minmax(0,1fr)]">
          <div className="flex flex-col justify-between px-7 py-9 text-white sm:px-9 lg:py-11">
            <div>
              <ClipboardCheckIcon
                className="h-7 w-7 text-[color:var(--color-si-lime)]"
                aria-hidden
              />
          ...
          <div className="divide-y divide-[color:var(--color-home-line)] bg-white">
            {page.items.map((item) => (
              <a
                key={item.title}
                href="#catalogue"
                className="group grid min-h-28 items-center gap-x-8 gap-y-1 px-6 py-6 transition-colors hover:bg-[color:var(--color-home-saf-bg)] sm:px-8 md:grid-cols-[110px_minmax(0,1fr)_auto]"
              >
      ...
      <section className="relative overflow-hidden bg-[color:var(--color-home-ink)]">
        <Image
          ...
          alt="Techniciens en repérage sur le terrain"
          fill
          sizes="100vw"
        ...
        <div className="relative mx-auto max-w-[var(--container,1280px)] px-6 py-16 md:px-8 lg:py-20">
          ...
          <div className="mt-12 grid gap-px overflow-hidden rounded-[14px] bg-white/12 md:grid-cols-3">
            {METHODE.map((temps, index) => (
              <div
                key={temps.etape}
                className="bg-[color:var(--color-home-ink)]/85 p-7 backdrop-blur-sm lg:p-8"
              >
      ...
      <BandeConfiance />
      <BandeCta
        titre="Quels contrôles pour votre projet ?"
        sousTitre="Le questionnaire identifie uniquement les diagnostics utiles — et leur prix."
        libelle="Identifier mes diagnostics"
      />
    </div>
  );
}
