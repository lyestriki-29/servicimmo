...
/** Échappe les valeurs saisies par le demandeur avant interpolation dans le HTML des emails. */
...
export function emailNotificationInterne(d: DevisInput): { subject: string; html: string } {
  const lignes = [
    ...
    ["Localisation", echapperHtml(d.localisation)],
    [
    ...
    ["Entreprise", echapperHtml(d.entreprise)],
    ["Contact", echapperHtml(`${d.nom} — ${d.emailPro} — ${d.telephone}`)],
    ["Message", d.message ? echapperHtml(d.message) : "—"],
  ]
    .map(
      ([k, v]) =>
...
export function emailAccuseReception(d: DevisInput): { subject: string; html: string } {
  return {
    ...
    html: cadre(
      `<h2 style="margin:0 0 12px;font-size:18px;">Demande bien reçue</h2><p>Bonjour ${echapperHtml(d.nom)},</p><p>Nous avons bien reçu votre demande concernant un chantier <strong>${LIBELLES_CHANTIER[d.typeChantier]}</strong> à ${echapperHtml(d.localisation)}. Notre équipe revient vers vous avec un devis chiffré sous 24 h ouvrées.</p><p style="margin-top:16px;">À très vite,<br/>L'équipe France Carottage</p>`,
    ),
  };
}
